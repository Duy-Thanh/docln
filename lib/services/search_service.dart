...
class SearchService {
  static const String baseUrl = "https://ln.hako.vn";
  ...
  Future<SearchResponse> search(String keyword, {int page = 1}) async {
    try {
      ...
      final url = "$baseUrl/tim-kiem?query=$encodedKeyword&keywords=$encodedKeyword&page=$page";
      ...
      if (noResultsElement != null && noResultsElement.text.contains("Không có truyện nào")) {
      ...
      for (var item in resultItems) {
        ...
        final chapterElement = item.querySelector('.thumb_attr.chapter-title a');
      ...
      final currentPageElement = document.querySelector('.paging_item.page_num.current');
      ...
      final paginationElements = document.querySelectorAll('.paging_item.page_num');
      if (paginationElements.isNotEmpty) {
        // Try to get last page from the "Cuối" link
        final lastPageLink = document.querySelector('.paging_item.paging_prevnext.next');
